# Android Security Hardening & Data Protection (Local Copy)
...
1. **Explicit Exports**: Every Activity, Service, Receiver in `AndroidManifest.xml` must explicitly state `android:exported="false"` unless an Intent Filter exposes it publicly.
2. **PendingIntents**: Always specify `PendingIntent.FLAG_IMMUTABLE` (or `FLAG_MUTABLE` only when external modification is strictly necessary).
3. **Deep Links**: Validate incoming deep link URI parameters before routing. Use App Links with `assetlinks.json` domain verification.
4. **Tapjacking / Overlay Protection**: Protect payment or authorization screens using: `view.filterTouchesWhenObscured = true`.
